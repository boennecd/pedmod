...
`-O3` in the `CXX11FLAGS` flags in your Makevars file may be useful.
...
The above seems to suggest that the randomized Korobov rules a preferable and 
...
We can also consider the convergence rate for the log likelihood. We also show 
...
to the desired number of samples. This will though imply the method cannot stop 
...
$$h_i = \left(1 + \sum_{k = 1}^K\sigma_k^2\right)\sigma_i^2\Leftrightarrow 
  \sigma_i^2 = 
