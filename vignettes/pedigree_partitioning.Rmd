...
### Potentialy Unconnected Partition

```{r pedigree_unconnected}
# without weights
partition <- get_unconnected_partition_pedigree(
  id = dat$id, father.id = dat$dad, mother.id = dat$mom, trace = 2L, 
  slack = .1)
...
# with weights
weighted_partition <- get_unconnected_partition_pedigree(
  id = dat$id, father.id = dat$dad, mother.id = dat$mom, trace = 2L, 
  slack = .1, id_weight = dat$id_weight, father_weight = dat$father_weight, 
  mother_weight = dat$mother_weight)
...
# show the partition
new_dat <- get_reduced_data(dat, weighted_partition$removed_edges)
ped <- with(new_dat, get_pedigree_mock(
  id = id, dadid = dad, momid = mom, sex = sex))
g_dat <- create_igraph_input(ped)
graph_fam <- graph.data.frame(g_dat, directed = FALSE)
...
# get the partition
system.time(
  max_part <- get_unconnected_partition(from = sim_dat$from, to = sim_dat$to, 
                                        trace = 1L, slack = .1, 
                                        max_kl_it_inner = 1000L))
max_part$balance_criterion # the balance criterion
NROW(max_part$removed_edges) # number of removed edges
...
# with random random initialization
...
system.time(
  max_part_shuffled <- get_max_balanced_partition(
...
system.time(
  max_part_shuffled_and_ordered <- get_max_balanced_partition(
    from = sim_dat$from, to = sim_dat$to, trace = 1L, slack = .1, 
