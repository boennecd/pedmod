...
where $Y_{ij}$ is the binary outcome of interest for indvidual $j$ in 
family/cluster $i$, $\vec x_{ij}$ is the indvidual's known covariates, 
...
the marginal log likelihood for these types of 
...
extend from a marginal log likelihood approximation to other approximations 
such as the derivatives, and added less precise but faster approximations 
of the $\Phi$ and $\Phi^{-1}$. Our own experince suggests that using the 
...
The approximation by @Genz02 have alrady been used to estimate these 
