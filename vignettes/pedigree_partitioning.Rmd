...
The relationship can also be represented as graph. For this purposes, we assign 
...
We have implemented procedures to do just this. We will return later to what 
...
```{r also_reduce_cost}
also_cut <- get_max_balanced_partition_pedigree(
  id = dat$id, father.id = dat$dad, mother.id = dat$mom, trace = 2L, 
  slack = .1)
also_cut$balance_criterion # the smallest of the two sets
removed_edges <- t(apply(also_cut$removed_edges, 1L, sort))
removed_edges # the relationships we have removed

# redo the plot
do_graph_plot(new_dat)
# color the vertices in the original graph instead
show_split(dat, also_cut)
```

We only removed  `r NROW(removed_edges)` relationships (edges) this time.

### Using Indviduals Weight (Vertex Weights)
...
actually observe. We can do by providing individual weights (vertex weights). 
...
weight of these individuals to a one and the remianing to some small positive 
number (say $\epsilon = 10^{-5}$):
...
weighted_partition <- get_max_balanced_partition_pedigree(
  id = dat$id, father.id = dat$dad, mother.id = dat$mom, trace = 2L, 
  slack = .1, id_weight = dat$id_weight)
weighted_partition$balance_criterion # the smallest of the two sets
removed_edges <- t(apply(weighted_partition$removed_edges, 1L, sort))
removed_edges # the relationships we have removed
...
We have colored the vertices which we observe in the above graph to make it easy 
...
suggested by @Hopcroft73 can be accesed through the 
`get_biconnected_components` and `get_block_cut_tree` function. 
...
partition 
above where each is of size 25. However, we cut `r NROW(removed_edges)` edges
...
in $\tilde V_1$ and define $I_2$ similarly. Then
 ...
 2. For $i = 1,\dots,\min(\text{max moves},\text{vertices in }B)$:
    ...
    * Consider all the vertices in $I_1 \setminus C_2$ and $I_2 \setminus C_1$
      ...
      $w(V) / 2 - \epsilon w(V) \leq w(V_1)$ and $w(V_1)\leq w(V) / 2 + \epsilon w(V)$ are 
      satisfied for some user defined $\epsilon \in [0, .5)$ and that the vertex has 
      not yet been used.
    * If there are no vertex then exit.
 ...
 3. If no vertex were moved then exit. Otherwise find the 
    $l = \text{argmax}_{k = 1,\dots},\sum_{i = 1}^k g_i$. 
...
We also limit the number of moves we make as @Karypis98.

$\epsilon$ is set with the `slack` argument of 
`get_max_balanced_partition` and the maximum number of moves to 
make in each iteration is set by the `max_kl_it_inner` argument of
`get_max_balanced_partition`. We illustrate the method below:

```{r partion_w_cut_simulate_small_graph}
# do the same but with also with the refinement step 
max_part <- get_max_balanced_partition(
