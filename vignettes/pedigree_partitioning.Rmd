...
in $\tilde V_1$ and define $I_2$ similarly. Then
 ...
 2. For $i = 1,\dots,\min(\text{max moves},\text{vertices in }B)$:
    ...
    * Consider all the vertices in $I_1 \setminus C_2$ and $I_2 \setminus C_1$
      ...
      satisfied for some user defined $\epsilon \in [0, .5)$ is satisfied 
...
# show the split
graph_fam <-  graph.data.frame(sim_dat, directed = FALSE)
V(graph_fam)$color <- "white"
nam <- vertex_attr(graph_fam)$name
V(graph_fam)$color[nam %in% max_part$set_1] <- "lightblue" 
V(graph_fam)$color[nam %in% max_part$set_2] <- "lightgray" 
par(mar = c(1, 1, 1, 1))
set.seed(1)
plot(graph_fam, layout = layout_with_kk, vertex.label = NA, 
     vertex.size = 3)
```
